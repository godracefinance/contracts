  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    // sell tokens
      ...
      if (!_whitelist[sender]) {
        _checkAndUpdatePeriod(sender, amount, "You can not sell this amount of tokens for the current period. Sorry, we kill bot");
      }
    // transfer tokens between addresses
      ...
      if (!_whitelist[sender] && !_whitelist[_msgSender()]) {
        _checkAndUpdatePeriod(sender, amount, "You can not transfer this amount of tokens for the current period. Sorry, we kill bot");
      }
    }
